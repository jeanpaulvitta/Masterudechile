...
// Reset password for an existing user in Supabase Auth
app.post("/make-server-000a47d9/auth/reset-password", async (c: any) => {
  try {
    const { email } = await c.req.json();
    ...
    const newPassword = `${Math.random().toString(36).substring(2, 15)}${Date.now()}`;
    ...
    if (updateError) {
      return c.json({ error: "Failed to reset password", details: updateError.message }, 500);
    }
...
app.post("/make-server-000a47d9/auth/create-user", async (c) => {
  try {
    ...
    if (createError) {
      ...
      if (errMsg.includes('already registered') || errMsg.includes('already been registered') || errMsg.includes('already exists')) {
